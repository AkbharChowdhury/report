...
public class OrderProcessor {
    public static List<Order> generateOrders(int numOrders) {
        ...
        for (int i = 0; i < numOrders; i++) {
            ...
            var order = new Order(orderId, amount, status);
    ...
    public static List<Order> filterHighValueCompletedOrders(List<Order> orders) {
        return orders.stream().filter(order -> "completed".equalsIgnoreCase(order.status()) && order.amount() > 200).toList();
    ...
    public static Map<String, Object> createSummaryReport(List<Order> orders) {
        ...
        summaryReport.put("total_amount", NumberFormat.getCurrencyInstance(Locale.UK).format(totalAmount));
